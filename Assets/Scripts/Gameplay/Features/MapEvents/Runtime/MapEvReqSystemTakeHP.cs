...
namespace Gameplay.MapEvents.Systems
{
    ...
    public sealed class MapEvReqSystemTakeHP : ISystem
    {
        public World World { get; set; }

        public Request<NormilizeMonsterInfoRequest> req_normilize;
        public Request<TakeHPRequest> req_take_hp;
        public void OnAwake()
        {
            req_take_hp =
                World.GetRequest<TakeHPRequest>();
            req_normilize =
        ...
        {
            foreach (var req in req_take_hp.Consume())
            {
                ProcessRequest(req);
            }
        }
        ...
        private void ProcessRequest(TakeHPRequest req)
        {
            ...
            foreach (var monster in tmp_copy_copy)
            {
                ...
                monster.current_hp = Mathf.Max(1, monster.current_hp);
